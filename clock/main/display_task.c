...
#define DEBUG (1)
    ...
    if (startsWithin12h && !alreadyFinished) {
        ...
        float const pxlsPerHr = CONFIG_CALCLOCK_WS2812_COUNT / hrsOnClock;
        ...
        for (uint pp = startPxl; pp < endPxl; pp++) {
            ...
            uint const pct = 100 - (pp - nowPxl) * 100 / CONFIG_CALCLOCK_WS2812_COUNT;
            uint const brightness = minBrightness + (maxBrightness - minBrightness) * pct/100;
            uint r, g, b;
            _hsv2rgb(*hue, 100, brightness, &r, &g, &b);
            uint pxlPos = pp % CONFIG_CALCLOCK_WS2812_COUNT;
            if (flipped) {
                pxlPos = (CONFIG_CALCLOCK_WS2812_COUNT - pxlPos) % 60;
            }
...
display_task(void * ipc_void)
{
    ...
    rmt_config_t config = RMT_DEFAULT_CONFIG_TX(CONFIG_CALCLOCK_WS2812_PIN, RMT_CHANNEL_0);
    config.clk_div = 2;  // set counter clock to 40MHz
    ESP_ERROR_CHECK(rmt_config(&config));
    ESP_ERROR_CHECK(rmt_driver_install(config.channel, 0, 0));
    led_strip_config_t strip_config = LED_STRIP_DEFAULT_CONFIG(CONFIG_CALCLOCK_WS2812_COUNT, (led_strip_dev_t)config.channel);
